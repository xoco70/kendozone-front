...
export class FakeBackendInterceptor implements HttpInterceptor {
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    const testUser = {id: 1, email: 'test', password: 'test', firstName: 'Test', lastName: 'User'};

    // wrap in delayed observable to simulate server api call
    return of(null).pipe(mergeMap(() => {

      // authenticate
      if (request.url.endsWith('https://api.kz-api.test/auth/login') && request.method === 'POST') {
      ...
      // get users
      if (request.url.endsWith('/tournaments') && request.method === 'GET') {
